#!/usr/bin/env bash
# Locate the DEP boundary budget with probing allocation and a 50% uniform floor.
...
LOG_DIR="$PROJECT_ROOT/base_exp/exp_edge/local_logs/dep_w4a4_floor50_probing_boundary_scan_$RUN_ID"
...
run_one() {
  ...
  local label="$3"
  local allocation_json="$ANALYSIS_DIR/dep_w4a4_floor50_probing_${label}_allocations.json"
  local run_name="ptq_pgfloor50_w4a4_out8_${label}_probing_fixedseed${SEED}_cal16_dep_e3_retry1"
...
PY
  ...
  if env \
      ...
      PTQ_RUN_SEED="$SEED" \
      PTQ_ALLOCATION_STRATEGY=probing_floor50 \
