#!/usr/bin/env bash
# Wait for the nominal-BOP pilot, then rerun non-uniform strategies at measured uniform BOP.
...
CORRECTED_JSON="$PROJECT_ROOT/base_exp/exp_edge/analysis/ptq/semeval_w4a4_probe_guided_outlier8_bopfix1_allocations.json"
RUN_ROOT="$PROJECT_ROOT/base_exp/exp_edge/runs/bert-large-uncased/semeval"
RUN_ID="${RUN_ID:-$(date +%Y%m%d_%H%M%S)}"
LOG_DIR="$PROJECT_ROOT/base_exp/exp_edge/local_logs/semeval_w4a4_probe_guided_out8_bopfix1_$RUN_ID"
...
while IFS=$'\t' read -r budget strategy scale target_bop weight_ratios act_ratios; do
  run_name="ptq_pg_w4a4_out8_${budget}_${strategy}_bopfix1_cal16_semeval_e10"
  ...
  if [[ -s "$run_dir/val_metrics.json" && \
    ...
    status=skipped_existing
      ...
      PTQ_ALLOCATION_STRATEGY="${strategy}_bopfix1" \
