#!/usr/bin/env bash
# Collect empirical Fisher sensitivity for one frozen task checkpoint.

set -euo pipefail

: "${TASK:?Set TASK}"
: "${BASE_RUN:?Set BASE_RUN}"
: "${FISHER_SEED:=20260806}"
: "${RUN_SUFFIX:?Set RUN_SUFFIX}"
...
RUN_NAME="fisher_stats_seed${FISHER_SEED}_cal16_${RUN_SUFFIX}"
OUTPUT_DIR="$EXP_DIR/runs/bert-large-uncased/$TASK/$RUN_NAME"
RUNCONFIG="$EXP_DIR/runconfigs/bert-large-uncased/$TASK/${RUN_NAME}.json"
STATS_PATH="$EXP_DIR/analysis/ptq/${TASK}_fisher_w4a4_seed${FISHER_SEED}_cal16.json"
...
export FISHER_CALIBRATION_BATCHES=16
...
"$PYTHON" "$JIANT_DIR/jiant/proj/main/runscript.py" run \
  ...
  --model_path "$EXP_DIR/runs/bert-large-uncased/$TASK/$BASE_RUN/best_model.p" \
