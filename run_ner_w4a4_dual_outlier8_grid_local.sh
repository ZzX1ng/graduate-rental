#!/usr/bin/env bash
# Run the full NER W4A4 W/A-outlier8 ratio grid sequentially.
...
LOG_DIR="$PROJECT_ROOT/base_exp/exp_edge/local_logs/ner_w4a4_dual_out8_grid_$RUN_ID"
...
run_one() {
  ...
  local run_name="ptq_oa_w4a4_dualout8_${label}_ms256_ner_e3"
...
case "${GRID_SHARD:-all}" in
  ...
  high)
    ...
    ;;
  *)
    echo "ERROR: GRID_SHARD must be all or high, got: $GRID_SHARD" >&2
    exit 2
    ;;
esac
