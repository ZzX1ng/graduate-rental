...
for layer in $(seq 0 24); do
  ...
  submit_gpu "tenney_dep_c${layer_padded}" \
...
summary_id="$(sbatch --parsable --dependency=afterok:"$dependency" --job-name=tenney_dep_sum \
  --export=ALL,TASK="$TASK",MAX_SEQ_LENGTH="$MAX_SEQ_LENGTH",EPOCHS="$EPOCHS" \
  "$SCRIPT_DIR/slurm_tenney_probe_summary.sbatch")"

echo "tenney_dep_sum -> $summary_id"
