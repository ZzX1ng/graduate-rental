#!/usr/bin/env bash
# Rerun all nominal-BOP allocations with one deterministic calibration seed.
...
LOG_DIR="$PROJECT_ROOT/base_exp/exp_edge/local_logs/semeval_w4a4_probe_guided_out8_fixedseed_$RUN_ID"
...
while IFS=$'\t' read -r budget strategy nominal_bop weight_ratios act_ratios; do
  run_name="ptq_pg_w4a4_out8_${budget}_${strategy}_fixedseed20260806_cal16_semeval_e10"
