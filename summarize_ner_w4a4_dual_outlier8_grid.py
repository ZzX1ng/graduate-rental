...
RUN_GLOB = os.path.join(
    PROJECT_ROOT,
    "base_exp/exp_edge/runs/bert-large-uncased/ner",
    "ptq_oa_w4a4_dualout8_*_ms256_ner_e3",
    "quant_summary.json",
)
OUTPUT = os.path.join(
    PROJECT_ROOT,
...
RUN_ROOT = os.path.join(
    PROJECT_ROOT, "base_exp/exp_edge/runs/bert-large-uncased/ner"
)


def load_major(run_name):
    path = os.path.join(RUN_ROOT, run_name, "quant_summary.json")
    with open(path, encoding="utf-8") as handle:
        data = json.load(handle)
    return data["val_metrics"]["ner"]["metrics"]["major"]
...
for path in glob.glob(RUN_GLOB):
    ...
    task_metrics = data["val_metrics"]["ner"]["metrics"]
...
w4a4_major = load_major("ptq_uniform_w4a4_ms256_ner_e3")
w8a8_major = load_major("ptq_uniform_w8a8_ms256_ner_e3")
print(f"W4A4_BASELINE={w4a4_major:.6f}")
print(f"W8A8_REFERENCE={w8a8_major:.6f}")
for row in rows:
    row["recovery_to_w8a8"] = (
        (row["major"] - w4a4_major) / (w8a8_major - w4a4_major)
    )

for threshold_name, threshold in (("W8A8", w8a8_major),):
    ...
    if eligible:
        ...
        print(f"MIN_COST_{threshold_name}=NONE")

low_candidates = [row for row in pareto if 0.15 <= row["recovery_to_w8a8"] < 1.0]
if low_candidates:
    chosen = min(low_candidates, key=lambda row: row["normalized_bop_overhead"])
    print(
        "LOW_CANDIDATE="
        f"W{chosen['weight_target_ratio'] * 100:g}%/A{chosen['activation_target_ratio'] * 100:g}% "
        f"major={chosen['major']:.6f} recovery={chosen['recovery_to_w8a8']:.6f} "
        f"overhead={chosen['normalized_bop_overhead']:.6f}"
    )
else:
    print("LOW_CANDIDATE=NONE")
